...
RUN --mount=type=cache,id=yingzo-api-gomod,target=/go/pkg/mod \
    go mod download
...
RUN --mount=type=cache,id=yingzo-api-gomod,target=/go/pkg/mod \
    --mount=type=cache,id=yingzo-api-gobuild,target=/root/.cache/go-build \
...
LABEL org.opencontainers.image.source="https://github.com/Zluo003/yingzo-api"

# Install runtime dependencies
# ffmpeg 提供 ffprobe：上传的参考素材要用它做可信媒体探测（尺寸/时长/编码），
# 参考视频时长也由它测量后参与计费。缺少它时参考视频会以 media_probe_failed 被拒。
...
RUN addgroup -g 1000 yingzo-api && \
    adduser -u 1000 -G yingzo-api -s /bin/sh -D yingzo-api

# Set working directory
WORKDIR /app

# Copy binary/resources with ownership to avoid extra full-layer chown copy
COPY --from=backend-builder --chown=yingzo-api:yingzo-api /app/yingzo-api /app/yingzo-api
COPY --from=backend-builder --chown=yingzo-api:yingzo-api /app/backend/resources /app/resources

# Create data directory
RUN mkdir -p /app/data && chown yingzo-api:yingzo-api /app/data

# Copy entrypoint script (fixes volume permissions then drops to yingzo-api)
...
# Run the application (entrypoint fixes /app/data ownership then execs as yingzo-api)
