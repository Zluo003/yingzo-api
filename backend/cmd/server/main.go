...
func main() {
	...
	// Migration-only mode. Needed on upgrades: the embedded migrations are only
	// applied during first-time setup, so an already-installed deployment would
	// otherwise keep running against a stale schema.
	if *migrateOnly {
