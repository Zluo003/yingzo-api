...
func (_q *APIKeyQuery) QueryVideoTasks() *VideoTaskQuery {
...
func (_q *APIKeyQuery) WithVideoTasks(opts ...func(*VideoTaskQuery)) *APIKeyQuery {
	query := (&VideoTaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withVideoTasks; query != nil {
		if err := _q.loadVideoTasks(ctx, query, nodes,
			func(n *APIKey) { n.Edges.VideoTasks = []*VideoTask{} },
			func(n *APIKey, e *VideoTask) { n.Edges.VideoTasks = append(n.Edges.VideoTasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadVideoTasks(ctx context.Context, query *VideoTaskQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *VideoTask)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(videotask.FieldAPIKeyID)
	}
	query.Where(predicate.VideoTask(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.VideoTasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
