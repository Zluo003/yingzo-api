...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		// 注意：这里不能再声明 models_list_config。迁移 235 已把该列重命名为
		// model_allowlist（语义升级为分组级模型白名单），143 建出的旧列在任何
		// 完整迁移过的库上都不存在；保留旧字段会让每个 groups 查询都以
		// `column "models_list_config" does not exist` 失败（列表与创建全部 500）。
