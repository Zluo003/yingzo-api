...
type CreateGroupRequest struct {
	...
	Description               string                        `json:"description"`
	Platform                  string                        `json:"platform" binding:"omitempty,oneof=anthropic openai gemini antigravity grok kimi zhipu deepseek minimax video composite"`
	...
	// 视频分组的按模型 × 分辨率每秒单价规则（仅 video 平台分组使用）。
...
type UpdateGroupRequest struct {
	...
	VideoPrice1080P              *float64                      `json:"video_price_1080p"`
	VideoModelPrices             map[string]map[string]float64 `json:"video_model_prices,omitempty"`
	// 视频分组的按模型 × 分辨率每秒单价规则；nil 表示本次不改动。
	VideoPricingRules               *[]service.VideoGroupPricingRule `json:"video_pricing_rules"`
	WebSearchPricePerCall           *float64                         `json:"web_search_price_per_call"`
	SearchPricePer1k                *float64                         `json:"search_price_per_1k"`
	AudioRealtimePricePerMin        *float64                         `json:"audio_realtime_price_per_min"`
