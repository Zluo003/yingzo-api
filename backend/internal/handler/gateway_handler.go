...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			if apiKey.Group.IsAgent() {
				if err := h.gatewayService.ValidateAgentLanguagePricing(c.Request.Context(), apiKey.Group, account, reqModel); err != nil {
					if accountReleaseFunc != nil {
						accountReleaseFunc()
					}
					reqLog.Warn("gateway.agent_pricing_unavailable",
						zap.Int64("account_id", account.ID), zap.String("model", reqModel), zap.Error(err))
	...
	for {
		...
		for {
			...
			// Agent 分组：价格 = 选中账号的源渠道价 × 该模型配置的倍率，缺一不可。
			// 必须在转发前失败，避免这单既收不到钱又要付上游成本。
			if currentAPIKey.Group.IsAgent() {
				if err := h.gatewayService.ValidateAgentLanguagePricing(c.Request.Context(), currentAPIKey.Group, account, reqModel); err != nil {
					if accountReleaseFunc != nil {
						accountReleaseFunc()
					}
					reqLog.Warn("gateway.agent_pricing_unavailable",
...
func defaultModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformVideo:
		// 视频分组只提供 Seedance 系列。缺少该分支会落到 default 分支，
		// 把 Claude 模型列表暴露给视频分组。
		return service.SupportedVideoModels()
...
func (h *GatewayHandler) mapUpstreamError(statusCode int) (int, string, string) {
	return service.MapUpstreamStatusToClientError(statusCode)
}
