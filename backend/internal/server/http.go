...
func ProvideHTTPServer(cfg *config.Config, router *gin.Engine) *http.Server {
	...
	if cfg.Server.H2C.Enabled {
		...
		// 等价于原 http2.ConfigureServer：标准库 HTTP2Config 承载其余 h2c 参数。
		// 标准库没有独立的 h2 空闲超时，h2 连接统一沿用 http.Server.IdleTimeout；
		// h2c 启用时以 h2c.idle_timeout 为准，保住 h2 路径原有的空闲回收语义。
		...
		server.HTTP2 = &http.HTTP2Config{
			...
			MaxReadFrameSize:              int(uint32(h2cConfig.MaxReadFrameSize)),
