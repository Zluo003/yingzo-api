...
// VideoProviderExtraKey 是视频账号在 extra 中记录上游平台（aigod/newtoken/mikuapi）的键。
const VideoProviderExtraKey = "video_provider"

// VideoModelResolutionsExtraKey 是视频账号在 extra 中记录「每个模型实际支持哪些
// 分辨率」的键，形如 {"seedance-2.0":["720p","1080p"]}。
//
// 为什么需要它：模型的官方分辨率集与具体上游渠道能提供的档位并不一致
// （同一模型在不同渠道可能只有 720p/1080p）。模型维度已由 model_mapping 白名单
// 约束，分辨率维度此前无处表达，只能按渠道默认能力放行，于是会被调度到该渠道
// 实际生成不了的分辨率上。
//
// 语义：键缺失或某模型未列出 => 该模型不限制分辨率（与旧行为一致）；
...
var supportedVideoProviders = []string{videoProviderAigod, videoProviderNewtoken, videoProviderMikuapi}
...
func NormalizeVideoProviderExtra(platform string, extra map[string]any) (map[string]any, error) {
	...
	if provider == "" {
		normalized[VideoProviderExtraKey] = videoProviderAigod
		return normalized, nil
	}
	for _, supported := range supportedVideoProviders {
		if provider == supported {
			normalized[VideoProviderExtraKey] = provider
			return normalized, nil
		}
	}
	return nil, infraerrors.BadRequest(
...
// normalizeVideoModelResolutionsExtra 就地校验并归一化分辨率白名单：
// 模型与分辨率都必须合法，值去重去空、按模型官方顺序排列，空条目直接删除
// （空列表等价于不限制，保留它只会让读取方多一种状态要判断）。
func normalizeVideoModelResolutionsExtra(extra map[string]any) error {
	raw, exists := extra[VideoModelResolutionsExtraKey]
	if !exists || raw == nil {
		delete(extra, VideoModelResolutionsExtraKey)
		return nil
	}
	byModel, ok := raw.(map[string]any)
	if !ok {
		return infraerrors.BadRequest(
			"invalid_video_model_resolutions",
			VideoModelResolutionsExtraKey+" must be an object mapping model to resolution list",
		)
	}
	normalized := make(map[string]any, len(byModel))
	for rawModel, rawResolutions := range byModel {
		model := strings.TrimSpace(rawModel)
		if !IsSupportedVideoModel(model) {
			return infraerrors.BadRequest(
				"invalid_video_model_resolutions",
				"Unsupported video model in "+VideoModelResolutionsExtraKey+": "+model,
			)
		}
		list, ok := rawResolutions.([]any)
		if !ok {
			return infraerrors.BadRequest(
				"invalid_video_model_resolutions",
				VideoModelResolutionsExtraKey+"."+model+" must be an array of resolutions",
			)
		}
		...
		for _, item := range list {
			...
			if !ok {
				return infraerrors.BadRequest(
					"invalid_video_model_resolutions",
					VideoModelResolutionsExtraKey+"."+model+" must contain only strings",
				)
			}
			canonical, ok := canonicalVideoResolution(model, value)
			if !ok {
				return infraerrors.BadRequest(
					"invalid_video_model_resolutions",
					"Resolution "+strings.TrimSpace(value)+" is not supported by "+model,
				)
			}
			allowed[canonical] = struct{}{}
		}
		// 按模型官方档位顺序回填，保证同一份配置序列化结果稳定（便于比对与测试）。
		ordered := make([]any, 0, len(allowed))
		for _, candidate := range SupportedVideoResolutions(model) {
		...
		normalized[model] = ordered
	}
	if len(normalized) == 0 {
		delete(extra, VideoModelResolutionsExtraKey)
		return nil
	}
	extra[VideoModelResolutionsExtraKey] = normalized
	return nil
}
...
func canonicalVideoResolution(model, resolution string) (string, bool) {
	target := strings.ToLower(strings.TrimSpace(resolution))
	if target == "" {
		return "", false
	}
	for _, candidate := range SupportedVideoResolutions(model) {
...
// videoAccountSupportsResolution 报告账号是否被允许服务该 (模型, 分辨率)。
// 未配置该模型的白名单时不限制——模型维度的准入由 model_mapping 负责。
func videoAccountSupportsResolution(account *Account, model, resolution string) bool {
	if account == nil || account.Extra == nil {
		return account != nil
	}
	raw := account.Extra[VideoModelResolutionsExtraKey]
	if raw == nil {
		return true
	}
	byModel, ok := raw.(map[string]any)
	if !ok {
		// 配置损坏时放行而不是拒绝：宁可多调度，也不要因一条脏数据让账号彻底不可用。
		return true
	}
	...
	canonical, ok := canonicalVideoResolution(model, resolution)
	if !ok {
		return false
	}
	for _, item := range list {
		if candidate, ok := item.(string); ok {
			if matched, ok := canonicalVideoResolution(model, candidate); ok && matched == canonical {
				return true
			}
		}
	}
...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	accountExtra, err = NormalizeVideoProviderExtra(input.Platform, accountExtra)
	if err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		normalizedExtra, err = NormalizeVideoProviderExtra(account.Platform, normalizedExtra)
		if err != nil {
			return nil, err
		}
