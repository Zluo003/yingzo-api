...
// normalizeCreateGroupInputForSimpleMode 丢弃 simple 模式下不该出现的字段：
// 只保留名称/描述/平台，倍率固定 1，订阅类型固定标准。
...
// normalizeUpdateGroupInputForSimpleMode 同理：simple 模式只允许改名称与描述。
...
func (s *adminServiceImpl) ListCompositeRoutes(ctx context.Context, groupID int64) ([]CompositeModelRoute, error) {
	// simple 模式下这些高级分组操作不允许直接调用（handler 层同样拦截，这里是纵深防御）。
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	// 白名单在创建路径同样收口：开启但为空、通配符位置非法都会 400。
	// （这段调用在早前的一次"清理死代码"里连同写入一起丢了，导致管理端保存白名单被静默丢弃。）
	...
	if platform == PlatformVideo && s.videoPricingRepo != nil {
		...
		// 回读而不是直接回填入参：Create 响应里的规则应带真实 id/时间戳，
		// 否则调用方拿到的是一组 id=0 的规则，与随后的 GET 不一致。
...
func (s *adminServiceImpl) hydrateVideoPricingRules(ctx context.Context, groups []Group) {
	...
	for i := range groups {
		if groups[i].Platform != PlatformVideo && !groups[i].IsAgent() {
			continue
		}
...
func (s *adminServiceImpl) hydrateVideoPricingRule(ctx context.Context, group *Group) {
	if group == nil || (group.Platform != PlatformVideo && !group.IsAgent()) || s.videoPricingRepo == nil {
		return
	}
