...
func agentTestImagePrices(resolutions ...string) []AgentModelPrice {
...
func TestEnsureAgentImageModelPriced(t *testing.T) {
	...
	model.Prices = agentTestImagePrices("2K")
	isImage, err = catalogService.EnsureAgentImageModelPriced(context.Background(), 9, PlatformGemini, "gemini-3-pro-image")
	require.True(t, isImage)
...
// 图片账号（账号管理里「添加图片账号」）走的就是这条路径：模型写在账号自己的
// model_mapping 上，目录同步时按模型名识别成图片，并且只有写了映射的账号会提供它
// —— 这就是"关联账号"的实现方式，不需要在目录里凭空加行。
func TestAgentCatalogDiscoversImageModelsFromAccountMapping(t *testing.T) {
	accounts := &agentCatalogAccountRepoStub{accounts: []Account{
		{
	...
	var discovered *AgentGroupModel
	for i := range config.Models {
		if config.Models[i].ModelCode == "gemini-3-pro-image" {
			discovered = &config.Models[i]
		}
	}
	require.NotNil(t, discovered, "账号映射里的图片模型必须被发现")
	require.Equal(t, AgentMediaTypeImage, discovered.MediaType)

	// 下游名 → 上游名由账号映射决定：请求这个模型时上游收到的是 nano-banana-pro。
	upstream, matched := accounts.accounts[0].ResolveMappedModel("gemini-3-pro-image")
	require.True(t, matched)
	require.Equal(t, "nano-banana-pro", upstream)

	// 没有写这条映射的第二个账号不提供该模型：候选池天然只剩账号 1。
