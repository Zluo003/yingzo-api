...
// schedulerSnapshotPlatforms 只覆盖 composite 路由可达的语言/图片平台。
// 视频平台不参与 composite 调度（CompositeRouteEndpoint 没有视频端点），
// 其账号由 video_service 直接按 group + platform 查询，因此有意不在此列。
