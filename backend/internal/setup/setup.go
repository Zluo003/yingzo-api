...
// RunMigrations 用已落盘的 config.yaml 应用尚未执行的 SQL 迁移后返回。
//
// 为什么需要它：迁移只在首次安装时被触发（main.go 里 AutoSetupFromEnv /
// SetupConfig 路径，前提是 NeedsSetup() 为真，即 config.yaml 与 .installed
// 都不存在）。实例一旦安装完成，升级到带新迁移的镜像时就没有任何入口会执行
// 迁移，schema 会静默落后于代码。--migrate 提供这个非交互入口。
func RunMigrations() error {
	...
	if seconds := getEnvIntOrDefault("SETUP_MIGRATION_TIMEOUT_SECONDS", 0); seconds > 0 {
		timeout = time.Duration(seconds) * time.Second
	}
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()

	logger.LegacyPrintf("setup", "applying database migrations from %s", GetDataDir())
	if err := repository.ApplyMigrations(ctx, db); err != nil {
		return fmt.Errorf("apply migrations: %w", err)
	}
	logger.LegacyPrintf("setup", "database migrations are up to date")
	return nil
}
