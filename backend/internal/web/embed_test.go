...
func TestEmbeddedFrontendBypassesBareVideoAPIRoutes(t *testing.T) {
	for _, path := range []string{
		...
		"/videos/extensions",
		"/videos/request-123",
		// 精确的 "/videos" 必须单独列出："/videos/" 前缀匹配不到它。
		"/videos",
	} {
...
// 引擎根路径上的 API 路由一旦漏进 SPA 回退，就会拿到 index.html + 200，
// 真实处理器永远不会执行。这里把 router 注册的全部根路径 API 列出来锁住。
func TestEmbeddedFrontendBypassesEveryRootAPIRoute(t *testing.T) {
	for _, path := range []string{
		"/health",
		"/models",
		"/responses",
		...
		"/setup/status",
		// 产物落到本地后返回给下游的媒体地址。
		"/media/00000000-0000-0000-0000-000000000000/asset.mp4",
		"/temporary-assets/token-abc",
		"/api/v1/accounts",
		"/v1/videos",
		"/v1beta/models",
	} {
...
// SPA 的客户端路由必须继续回退到 index.html，否则刷新页面会 404。
func TestEmbeddedFrontendStillServesSPARoutes(t *testing.T) {
	for _, path := range []string{
		"/",
		"/login",
		"/admin/accounts",
