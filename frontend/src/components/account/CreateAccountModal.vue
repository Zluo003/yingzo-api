<template>
  ...
  >
    ...
    >
      ...
      <div v-if="!isVideoMode">
        ...
        <div class="mt-2 flex flex-wrap rounded-lg bg-gray-100 p-1 dark:bg-dark-700" data-tour="account-form-platform">
          ...
          >
            ...
            {{ t('admin.groups.platforms.video') }}
          </button>
        </div>
        <!-- Multi-protocol API-key providers: Kimi / Zhipu GLM / DeepSeek / MiniMax -->
          ...
          <div class="mt-2 space-y-3">
            <div v-for="model in selectedVideoModels" :key="model">
              ...
              <div class="mt-1.5 flex flex-wrap gap-2">
                <button
                  v-for="resolution in videoModelResolutions(model)"
                  :key="resolution"
                  type="button"
      ...
      <!-- API Protocol Selection (Kimi / Zhipu / DeepSeek / MiniMax) -->
          ...
          <template v-else>
            ...
            <div v-if="modelRestrictionMode === 'whitelist'">
              <ModelWhitelistSelector
                v-model="allowedModels" :platform="form.platform" :sync-credentials="syncPreviewCredentials"
      ...
      <div v-if="form.platform === 'anthropic' && accountCategory === 'bedrock'" class="space-y-4">
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
          ...
          <div v-if="modelRestrictionMode === 'whitelist'">
            <ModelWhitelistSelector
              v-model="allowedModels" platform="anthropic" :sync-credentials="syncPreviewCredentials"
              @upstream-synced="upstreamModelsPreviewed = true"
            />
            <p class="text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.selectedModels', { count: allowedModels.length }) }}
      ...
      <!-- OpenAI API 长上下文计费开关（所选分组已全部启用按档计费时隐藏冗余开关） -->
      <div
      ...
      <!-- OpenAI APIKey 生图：把上游返回的图片 URL 回填为 b64_json -->
      <div
...
// 上游请求标识响应头：上游供应商用它回传请求 ID，留空则不记录。
...
const apiKeyBaseUrlPlaceholder = computed(() => {
  ...
  switch (form.platform) {
    ...
    case 'video':
      return videoProviderDefaults.value.baseUrl
    default:
...
// 所选分组已全部启用长上下文按档计费时，账号级开关冗余（分组侧已决定计费档）。
...
// ===== 国产供应商（Kimi / Zhipu GLM / DeepSeek / MiniMax）多协议支持 =====
// 账号类型（payg / coding）与 API 协议（adaptive / 各原生协议）正交：
// adaptive 会把三种原生协议的端点都写进 credentials.api_base_urls，由后端按入站协议路由。
...
const isMultiProtocolPlatform = computed(() => isCNPlatform.value)
...
const cnAdaptiveProtocolOptions = computed<
  Array<{ value: CnNativeApiProtocol; labelKey: string }>
>(() => {
...
watch(accountMode, (mode, previousMode) => {
  ...
  if (apiProtocol.value === 'adaptive') {
    ...
    for (const item of cnAdaptiveProtocolOptions.value) {
      if (
        !adaptiveBaseUrls.value[item.value] ||
        adaptiveBaseUrls.value[item.value] === previousDefaults[item.value]
      ) {
...
/**
 * 上游平台预设。取值必须与后端 videoDefaultBaseURLForProvider /
 * videoAccountDefaultDuration 保持一致，否则界面默认值会和真实请求不一致。
 */
const videoProviderDefaultsMap: Record<
  VideoProvider,
  {
    baseUrl: string
    apiPath: string
...
> = {
  aigod: {
    ...
    apiPath: '/v1/videos',
    pollIntervalMs: 2000,
    // 与后端 videoDefaultPollTimeout 一致：aigod / newtoken 统一 15 分钟。
...
const videoProviderOptions: {
  ...
  {
    value: 'aigod',
    labelKey: 'admin.accounts.video.providers.aigod',
    hintKey: 'admin.accounts.video.aigodAdapter'
  },
  {
    value: 'newtoken',
...
const resolvedVideoAPIPath = computed(
  () => videoAPIPath.value.trim() || videoProviderDefaults.value.apiPath
)
const upstreamBillingAutoProbeEnabled = ref(true)
const upstreamBillingProbeSupported = computed(() => form.platform !== 'video')

const syncPreviewCredentials = computed(() => {
  if (!apiKeyValue.value) return undefined
  // adaptive 协议下预览必须走用户实际编辑的 chat completions 端点。
  const baseUrl =
...
/** 预览过上游模型 / 显式配了具体映射的账号：创建后需要落一次能力元数据同步。 */
...
/** 未勾选的模型不参与提交；全部模型都没有勾选时返回 undefined（不写该键）。 */
...
const toggleVideoModel = (model: string) => {
  // 保持勾选顺序稳定为 videoDefaultModels 的顺序，避免提交结果抖动。
  const next = selectedVideoModels.value.filter((item) => item !== model)
  if (next.length === selectedVideoModels.value.length) {
    next.push(model)
  }
...
// OpenAI APIKey 生图：把上游返回的图片 URL 回填成 b64_json（后端做转换）
...
const submitCreateAccount = async (payload: CreateAccountRequest) => {
  ...
  try {
    ...
    const hasConcreteMappedTarget =
      ...
      modelMapping !== null &&
      Object.values(modelMapping).some(
        (target) => typeof target === 'string' && target.trim() !== '' && !target.includes('*')
      )
