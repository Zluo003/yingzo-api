...
const editVideoDefaultsMap: Record<
  VideoProvider,
...
const resolvedEditVideoAPIPath = computed(
  () => editVideoAPIPath.value.trim() || editVideoProviderDefaults.value.apiPath
)
...
const defaultBaseUrl = computed(() => {
  ...
  // 视频账号按所选上游取默认地址；缺少该分支会在 base_url 清空时把
  // Anthropic 的地址写进 extra.base_url。
  if (props.account?.platform === 'video') return editVideoProviderDefaults.value.baseUrl
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (newAccount.platform === 'video') {
      ...
      editBaseUrl.value = (extra?.base_url as string) || (credentials.base_url as string) || platformDefaultUrl
    } else if (isCNApiKeyAccount.value && editApiProtocol.value === 'adaptive') {
      editBaseUrl.value = editAdaptiveBaseUrls.value.chat_completions
    } else {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      // 视频平台不支持上游计费探测；发送该标记会把运营已启用的探测关掉。
