...
export type GroupPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'grok' | 'kimi' | 'zhipu' | 'deepseek' | 'minimax' | 'video' | 'composite'

export type VideoModelPrices = Record<string, Record<string, number>>

/**
 * 视频分组按 模型 × 分辨率 的每秒单价规则（对应后端 video_group_pricing_rules）。
 * credits_per_second 在管理界面按“元/秒”录入与展示。
 */
...
export interface Group {
  ...
  // 视频分组按 模型 × 分辨率 的每秒单价规则（仅 video 平台分组使用）。
...
export type AccountPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'grok' | 'kimi' | 'zhipu' | 'deepseek' | 'minimax' | 'video'
