  ...
  <AppLayout>
    ...
    >
      ...
      >
        ...
        <!-- 视频计费规则（仅 video 平台分组）：按 模型 × 分辨率 配置每秒单价 -->
        <div v-if="createForm.platform === 'video'" class="border-t pt-4">
          ...
          <p
            v-if="createForm.video_pricing_rules.length === 0"
            class="mb-2 text-xs text-amber-600 dark:text-amber-400"
          >
            {{ t("admin.groups.videoPricing.empty") }}
          </p>
          <div
            v-for="(rule, index) in createForm.video_pricing_rules"
            :key="`video-rule-${index}`"
            class="mb-2 grid grid-cols-1 items-end gap-2 md:grid-cols-[1fr_1fr_1fr_auto_auto]"
          >
            <div>
              <label class="input-label">{{ t("admin.groups.videoPricing.model") }}</label>
              <select v-model="rule.model_code" class="input" @change="onVideoPricingModelChange(createForm, index)">
                <option v-for="model in VIDEO_PRICING_MODELS" :key="model" :value="model">
                  {{ model }}
                </option>
              </select>
            </div>
            <div>
              ...
              <select v-model="rule.resolution" class="input">
                <option
                  v-for="resolution in videoPricingResolutions(rule.model_code)"
                  :key="resolution"
                  :value="resolution"
                >
                  {{ resolution }}
            ...
            <div>
              <label class="input-label">
                {{ t("admin.groups.videoPricing.creditsPerSecond") }}（{{ t("admin.groups.videoPricing.priceUnit") }}）
              </label>
              <input
                v-model.number="rule.credits_per_second"
                type="number"
                min="0"
                step="0.0001"
                class="input"
                :placeholder="t('admin.groups.videoPricing.pricePlaceholder')"
              />
            </div>
            ...
            <button
              ...
              @click="removeVideoPricingRule(createForm, index)"
            >
              {{ t("admin.groups.videoPricing.removeRule") }}
            </button>
          </div>
          <p v-if="hasDuplicateVideoPricingRule(createForm.video_pricing_rules)" class="mb-2 text-xs text-red-600 dark:text-red-400">
            {{ t("admin.groups.videoPricing.duplicate") }}
          </p>
          <p class="mb-2 text-xs text-gray-500 dark:text-gray-400">
            {{ t("admin.groups.videoPricing.referenceBillingHint") }}
          </p>
          <button type="button" class="btn btn-secondary" @click="addVideoPricingRule(createForm)">
    ...
    >
      ...
      >
        ...
        <div v-if="editForm.platform === 'video'" class="border-t pt-4">
          ...
          <p
            v-if="editForm.video_pricing_rules.length === 0"
            class="mb-2 text-xs text-amber-600 dark:text-amber-400"
          >
            {{ t("admin.groups.videoPricing.empty") }}
          </p>
          ...
          >
            <div>
              <label class="input-label">{{ t("admin.groups.videoPricing.model") }}</label>
              <select v-model="rule.model_code" class="input" @change="onVideoPricingModelChange(editForm, index)">
                <option v-for="model in VIDEO_PRICING_MODELS" :key="model" :value="model">
                  {{ model }}
                </option>
              </select>
            ...
            <button
              type="button"
              class="btn btn-secondary mb-1"
              @click="removeVideoPricingRule(editForm, index)"
            >
              {{ t("admin.groups.videoPricing.removeRule") }}
            </button>
          </div>
          <p v-if="hasDuplicateVideoPricingRule(editForm.video_pricing_rules)" class="mb-2 text-xs text-red-600 dark:text-red-400">
            {{ t("admin.groups.videoPricing.duplicate") }}
          </p>
          <p class="mb-2 text-xs text-gray-500 dark:text-gray-400">
            {{ t("admin.groups.videoPricing.referenceBillingHint") }}
          </p>
          <button type="button" class="btn btn-secondary" @click="addVideoPricingRule(editForm)">
            {{ t("admin.groups.videoPricing.addRule") }}
          </button>
        </div>
        <!-- 高峰时段倍率配置（仅订阅类型分组） -->
...
const compositeRoutePlatformOptions = computed(() =>
  // video 平台只服务异步视频端点，不是 composite 路由的目标平台：
  // 后端 composite_model_routes.target_platform 的 CHECK 也不含 video。
  CONCRETE_PLATFORM_OPTIONS.filter((option) => option.value !== "video"),
);
...
const createForm = reactive({
  ...
  // 视频计费规则（仅 video 平台分组）：按 模型 × 分辨率 的每秒单价
  video_pricing_rules: [] as VideoPricingRuleRow[],
...
const handleCreateGroup = async () => {
  ...
  // 视频分组的计费规则：必须能在前端拦住重复组合与非法价格，
  // 否则会以数据库唯一约束错误的形式返回。
  let createVideoPricingRules: ReturnType<typeof serializeVideoPricingRules> = [];
  if (createForm.platform === "video") {
    if (hasDuplicateVideoPricingRule(createForm.video_pricing_rules)) {
      appStore.showError(t("admin.groups.videoPricing.duplicate"));
      return;
    }
    createVideoPricingRules = serializeVideoPricingRules(
      createForm.video_pricing_rules,
    );
    if (createVideoPricingRules === null) {
      appStore.showError(t("admin.groups.videoPricing.invalidPrice"));
      return;
    }
  }
  ...
  try {
    const {
      ...
      video_pricing_rules: _createFormVideoPricingRules,
      ...createGroupForm
    } = createForm;
    ...
    const requestData = {
      ...
      ...(createForm.platform === "video"
        ? { video_pricing_rules: createVideoPricingRules }
        : {}),
...
const handleEdit = async (group: AdminGroup) => {
  ...
  editForm.video_pricing_rules = createVideoPricingRulesForm(
    group.video_pricing_rules,
  );
...
const handleUpdateGroup = async () => {
  ...
  // 视频分组的计费规则：重复组合会撞后端唯一约束，非法价格会 400，先在前端拦下。
  let editVideoPricingRules: ReturnType<typeof serializeVideoPricingRules> = [];
  if (editForm.platform === "video") {
    if (hasDuplicateVideoPricingRule(editForm.video_pricing_rules)) {
      appStore.showError(t("admin.groups.videoPricing.duplicate"));
      return;
    }
    editVideoPricingRules = serializeVideoPricingRules(
      editForm.video_pricing_rules,
    );
    if (editVideoPricingRules === null) {
      appStore.showError(t("admin.groups.videoPricing.invalidPrice"));
      return;
    }
  }
  ...
  try {
    ...
    // video_pricing_rules 在表单里是编辑态行结构，必须排除后再按后端形状写回。
    const {
      video_pricing_rules: _editFormVideoPricingRules,
      ...editFormPayload
    } = editForm;
    const payload = {
      ...editFormPayload,
      ...
      ...(editForm.platform === "video"
        ? { video_pricing_rules: editVideoPricingRules }
        : {}),
      fallback_group_id:
    ...
    // 表单行是编辑态的中间结构，请求体只接受后端 VideoGroupPricingRule 形状。
    if (editForm.platform === "video") {
      payload.video_pricing_rules = editVideoPricingRules ?? [];
    } else {
      delete (payload as Record<string, unknown>).video_pricing_rules;
    }
    // v-model.number 清空输入框时产生 ""，转为 null 让后端设为无限制
    const emptyToNull = (v: any) => (v === "" ? null : v);
