...
describe('channel monitor Grok provider', () => {
  ...
  it('offers Grok in the responsive provider grid and prefills its official defaults', async () => {
    ...
    // 断言"渲染出的 provider 集合 == 共享目录 PROVIDERS"，而不是写死数字：
    // 新增 provider（如 minimax）时这里不该失败，但两边出现分歧必须被抓到。
    // 用集合而非顺序比较——弹窗的展示顺序与 PROVIDERS 的声明顺序本来就不同。
    const renderedProviders = providerButtons
      .map((button) =>
        (button.attributes('data-testid') ?? '').replace('monitor-provider-', '')
      )
      .sort()
    expect(renderedProviders).toEqual([...PROVIDERS].sort())
